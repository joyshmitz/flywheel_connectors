...
fn load_schema() -> Validator {
    let schema: Value = serde_json::from_str(FZPF_V01_SCHEMA)
        .expect("FZPF schema should be valid JSON");
...
fn reject_too_many_input_constraints() {
    ...
    let constraints: Vec<String> = (0..65)
        .map(|i| format!(r#"{{ "pointer": "/field{}", "op": "eq", "value": {} }}"#, i, i))
        .collect();
...
fn schema_validation_is_deterministic() {
    ...
    let results: Vec<bool> = (0..100)
        .map(|_| validator.validate(&doc).is_ok())
        .collect();
