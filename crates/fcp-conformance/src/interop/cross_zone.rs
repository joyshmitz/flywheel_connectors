...
fn test_approval_token_chain() -> Result<(), String> {
    ...
    let mismatched_chain = vec![token_work_to_private.clone(), token_public_to_work.clone()];
...
fn zone_level(zone: &str) -> u8 {
    match zone {
        ...
        "z:community" => 1,
        "z:public" => 0,
        _ => 0, // Unknown zones treated as public
    }
}
...
mod tests {
    ...
    fn test_chain_validation() {
        ...
        // Broken chain
        assert!(!is_chain_valid(&[t2.clone(), t1.clone()], "a", "c"));
    }
}
