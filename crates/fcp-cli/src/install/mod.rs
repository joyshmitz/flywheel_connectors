...
use fcp_registry::{
    manifest_signing_bytes, signature_message, ConnectorBundle, ConnectorTarget, MirrorResult,
    RegistryError, RegistryTrustPolicy, RegistryVerifier, VerifiedConnectorBundle,
...
fn parse_target_triple(triple: &str) -> ConnectorTarget {
    ...
    let os = if parts.len() >= 3 { parts[2] } else { "unknown" };
    ConnectorTarget {
        os: os.to_string(),
        arch: arch.to_string(),
    }
...
fn registry_error_to_install_error(connector_id: &str, err: RegistryError) -> InstallError {
    match err {
        ...
        RegistryError::SignatureInvalid { kid } => {
            InstallError::signature_verification_failed(connector_id, &format!("invalid signature for key {kid}"))
        }
        ...
        RegistryError::RegistrySignatureRequired => {
            InstallError::signature_verification_failed(connector_id, "registry signature required but missing")
        }
...
fn generate_demo_manifest(connector_id: &str, version: &str, binary: &[u8], keys: &DemoKeys) -> String {
...
"#
    ...
    let base_manifest = manifest_template.replace(PLACEHOLDER_INTERFACE_HASH, &computed_hash.to_string());
    ...
    let signing_bytes = manifest_signing_bytes(&manifest_for_signing)
        .expect("compute signing bytes");
...
fn verify_bundle(
    ...
    publisher_keys.insert("demo-publisher".to_string(), demo_keys.verifying_key.clone());
...
async fn mirror_to_store(
    ...
    // Parse zone ID
    let zone_id: ZoneId = zone.parse().map_err(|e| {
        RegistryError::ManifestParse(fcp_manifest::ManifestError::ZoneId(e))
    })?;

    // Create object ID key from demo key (in reality, this comes from zone config)
    let object_id_key = ObjectIdKey::from_bytes(demo_keys.signing_key.to_bytes()[..32].try_into().unwrap_or([0u8; 32]));
