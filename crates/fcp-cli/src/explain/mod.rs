...
use types::{
    reason_code_description, DecisionOutcome, EvidenceItem, EvidenceType, ExplainError,
    ExplainReport, SignerInfo,
};
    ...
    ExplainReport {
        ...
        evidence: vec![
            ...
            EvidenceItem {
                ...
                description: "Zone policy denies access from z:external to z:sensitive"
                    .to_string(),
            },
        ],
        explanation: Some(
...
fn output_error(error: &ExplainError, json: bool) -> Result<()> {
    if json {
        let output = serde_json::to_string_pretty(error)
            .context("failed to serialize error to JSON")?;
        println!("{output}");
...
fn print_human_readable(report: &ExplainReport) {
    ...
    // Signature info
    println!("Signed by:    {} at {}", report.signed_by.node_id,
        chrono::DateTime::from_timestamp(report.signed_by.signed_at as i64, 0)
            .map(|dt| dt.format("%Y-%m-%d %H:%M:%S UTC").to_string())
            .unwrap_or_else(|| report.signed_by.signed_at.to_string()));
    println!();
}
