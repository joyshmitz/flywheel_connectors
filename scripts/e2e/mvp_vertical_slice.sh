...
SCRIPT_NAME="e2e_mvp_vertical_slice"
...
SCENARIOS=(
  "happy_path|${SCRIPT_DIR}/happy_path.sh|Install invoke receipt audit verify|true"
  "denial_path|${SCRIPT_DIR}/denial_path.sh|Invoke without cap -> DecisionReceipt -> explain|true"
  "revocation_flow|${SCRIPT_DIR}/revocation_flow.sh|Issue token -> revoke -> deny|true"
  "taint_approval|${SCRIPT_DIR}/taint_approval.sh|Tainted input -> approval -> success|true"
  "offline_repair|${SCRIPT_DIR}/offline_repair_flow.sh|Reduced availability -> repair -> recovery|true"
  "epoch_replay|${SCRIPT_DIR}/epoch_replay_mirror_install.sh|Binary mirror install + epoch replay|true"
)
...
echo "MVP vertical slice complete. Summary: ${SUMMARY_JSON}"
