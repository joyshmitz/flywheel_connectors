...
step_run_routing_tests() {
  cargo test -p fcp-mesh --test mesh_integration routing -- --nocapture
}
...
run_step \
  "run_routing_tests" \
  2 \
  '{"crate":"fcp-mesh","target":"mesh_integration","filter":"routing"}' \
  "" \
  '{"category":"routing","purpose":"selection_rules"}' \
  step_run_routing_tests
