...
SCRIPT_NAME="e2e_admission_control"
SEED="0xADMU"
...
step_run_admission_tests() {
  cargo test -p fcp-mesh --test mesh_integration admission_control -- --nocapture
}
...
run_step \
  "run_admission_control_tests" \
  2 \
  '{"crate":"fcp-mesh","target":"mesh_integration","filter":"admission_control"}' \
  "" \
  '{"category":"admission_control","purpose":"budget_limits"}' \
  step_run_admission_tests
