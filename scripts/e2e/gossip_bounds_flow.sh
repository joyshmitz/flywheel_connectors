...
step_run_gossip_tests() {
  cargo test -p fcp-mesh --test mesh_integration gossip_ -- --nocapture
}
...
run_step \
  "run_gossip_integration_tests" \
  2 \
  '{"crate":"fcp-mesh","target":"mesh_integration","filter":"gossip_"}' \
  "" \
  '{"category":"gossip","purpose":"config_bounds"}' \
  step_run_gossip_tests
