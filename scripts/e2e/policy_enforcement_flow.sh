...
SCRIPT_NAME="e2e_policy_enforcement"
SEED="0xP0L1"
...
step_run_policy_tests() {
  cargo test -p fcp-mesh --test mesh_integration policy_enforcement -- --nocapture
}
...
run_step \
  "run_policy_enforcement_tests" \
  2 \
  '{"crate":"fcp-mesh","target":"mesh_integration","filter":"policy_enforcement"}' \
  "" \
  '{"category":"policy_enforcement","purpose":"decision_rules"}' \
  step_run_policy_tests
